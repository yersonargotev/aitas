...
const getProjectNotesKey = (projectId: string): string =>
	`project_notes_${projectId}`;

export function getNotesFromStorage(projectId: string): Note[] {
	if (typeof window === "undefined") return [];
	try {
		const notesJson = localStorage.getItem(getProjectNotesKey(projectId));
...
export function saveNoteToStorage(
	noteToSave: Partial<Note> & { projectId: string },
): Note {
	...
	const notes = getNotesFromStorage(noteToSave.projectId);
	const now = new Date().toISOString();
	let finalNote: Note;
	...
	try {
		localStorage.setItem(
			getProjectNotesKey(noteToSave.projectId),
...
export function deleteNoteFromStorage(noteId: string, projectId: string): void {
	...
	try {
		localStorage.setItem(getProjectNotesKey(projectId), JSON.stringify(notes));
	} catch (error) {
